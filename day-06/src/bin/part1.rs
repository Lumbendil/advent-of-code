...
fn line_to_numbers(line: String) -> Vec<i64> {
    line.trim().split(' ')
        .filter(|x| !x.is_empty())
...
fn process(mut input: impl BufRead, mut output: impl Write) {
    ...
    let value: i64 = times.iter().zip(distances.iter())
        .map(|(time, distance)| {
            ...
            let sol_1 = ((time_f + (time_f.powi(2) - 4.0 * distance_f).sqrt()) / 2.0 - 1.0).ceil() as i64;
            let sol_2 = ((time_f - (time_f.powi(2) - 4.0 * distance_f).sqrt()) / 2.0 + 1.0).floor() as i64;
